...
app.get("/adminOnlineOrders", (req,res)=>{
    if(req.session.user && req.session.user.role==='Admin'){
        ...
        db.query(q, (err,data)=>{
            if(err) res.json(err)
            else res.render('adminOnlineOrders', {user:req.session.user, data:data})
        })
    }
    else res.json('You do not have access to this page')
})
...
app.get("/adminDineInOrders", (req,res)=>{
    if(req.session.user && req.session.user.role==='Admin'){
        ...
        db.query(q, (err,data)=>{
            if(err) res.json(err)
            else res.render('adminDineInOrders', {user:req.session.user, data:data})
        })
    }
    else res.json('You do not have access to this page')
})
...
app.get("/adminReservations", (req,res)=>{
    if(req.session.user && req.session.user.role==='Admin'){
        ...
        db.query(q, (err,data)=>{
            if(err) res.json(err)
            else res.render('adminReservations', {user:req.session.user, data:data})
        })
    }
    else res.json('You do not have access to this page')
})
...
app.get("/adminDashboard", (req,res)=>{
    if(req.session.user && req.session.user.role==='Admin'){
        ...
        const dateR = new Date().getFullYear() + '-' + ("0" + (new Date().getMonth() + 1)).slice(-2) + '-' + ("0" + (new Date().getDate())).slice(-2) 
                    ...
                    if(err) res.json('error')
                        ...
                        db.query(q3, (err,data3)=>{
                            if(err) res.json('error')
                                ...
                                res.render('adminDashboard', {user:req.session.user, data1:data1, data2:data2, data3:data3, info:info})
                            }
                        })
                    }
                })
            ...
            if(err) res.json(err)
                ...
                db.query(q2, (err, data)=>{
                    if(err) res.json(err)
                        ...
                        const q3 = "INSERT INTO dineIns (customer_id, address, description, price_breakdown, total, createdAt, instruction, status) VALUES (?)";
...
app.post("/checkout", (req,res)=>{
    if(!req.session.user){
        ...
        const q3 = "INSERT INTO onlineOrders (customer_id, address, description, price_breakdown, total, createdAt, instruction, status) VALUES (?)";
...
app.post("/makeReservation", (req,res)=>{
    ...
    db.query(q1, (err,data)=>{
        if(err || data.length===0) res.json('Sorry, table not available')
            ...
            const q3 = "INSERT INTO reservations (table_id, customer_id, guests, time) VALUES (?)";
            const values = [
                data[0].id,
                req.session.user.id,
                req.body.guests,
